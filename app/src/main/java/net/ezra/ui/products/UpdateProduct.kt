...
class ProductViewModel : ViewModel() {
    ...
    val productState = MutableLiveData<Vehicle>()

    fun getProduct(productId: String) {
        viewModelScope.launch {
            val document = firestore.collection("products").document(productId).get().await()
            val product = document.toObject(Vehicle::class.java) ?: return@launch
            productState.value = product
        }
    }

    fun updateProduct(product: Vehicle) {
